...
def test_split_maps_to_zero_cost() -> None:
    rows = [_row(op="ETF份额分拆", qty=Decimal(2000), price=Decimal(0), amount=Decimal(0),
                 cash_delta=Decimal(0), fee=Decimal(0), share_balance=Decimal(4000))]
    result = convert(rows, final_cash=Decimal("100.000"))
    assert result.rows[0]["操作"] == "拆股"
    assert result.rows[0]["成交价格"] == "0"
    # 拆股不影响现金：初始现金 = 期末现金
