...
def _sse(*deltas: str, usage: dict[str, Any] | None = None) -> httpx.Response:
    """构造 SSE 流式响应体（content 增量 + 可选 usage 末块 + [DONE]）。"""
    lines = [
...
async def test_call_native_success_sends_response_format(monkeypatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return _sse('{"summary": "好", ', '"evidence_ids": []}',
    ...
    assert resp.audit.completion_tokens == 5
...
async def test_call_falls_back_on_4xx_unsupported_schema(monkeypatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return _sse('{"summary": "兜底", ', '"evidence_ids": []}')
