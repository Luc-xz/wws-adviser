...
def test_intraday_returns_suspend_without_calibrated_signal(migrated_client: TestClient) -> None:
    """当前无已校准信号 + 账本未对账 → 按规范返回暂停建议 + 原因码（不静默）。"""
    headers = _login(migrated_client)
    r = migrated_client.post("/api/v1/assistant/intraday", json={"code": "600519"},
                             headers=headers)
    assert r.status_code == 200
    ...
    assert "no_calibrated_signal" in advice["reasons"]
    # 有效期语义：expires_at > valid_from
    assert advice["expires_at"] > advice["valid_from"]
    # 调整轨迹字段存在于 payload（发布形态展示折扣/约束链）
    assert isinstance(advice["trail"], list)


def test_intraday_persists_advice_record(migrated_client: TestClient) -> None:
    """FR-REV-003：建议落库（数据快照 + 原因链），供观察窗口评价回读。"""
    headers = _login(migrated_client)
    r = migrated_client.post("/api/v1/assistant/intraday", json={"code": "600519"},
                             headers=dict(headers, **{"Idempotency-Key": "intraday-persist-1"}))
    assert r.status_code == 200
    advice_id = r.json()["advice"]["advice_id"]
    ...
    with migrated_client.app.state.session_factory() as db:
        row = db.scalar(select(AdviceRecord).where(AdviceRecord.id == advice_id))
        assert row is not None
        assert row.action == "suspend" and row.state == "degraded"
        assert "no_calibrated_signal" in (row.reasons_json or "")
