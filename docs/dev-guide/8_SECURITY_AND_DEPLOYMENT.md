...
| 事项 | 确认值/策略 | 性质 |
| --- | --- | --- |
| CSRF 策略 | **双提交 Cookie Token + SameSite**；高敏感操作（导入确认/删除等）额外 Origin 校验 | 已确认 |
| 备份异地加密 | 客户端可选加密，异地启用，密钥与备份分离 | 运行配置 |

### 12.2 部署与供应商（全部已确认 2026-08-12）

> 全部供应商选型已完成，无待确认项。

> **已确认（2026-08-11/12）**：
> - **VPS 地域：国内 VPS**（阿里云/腾讯云/华为云等国内节点）。数据源（AKShare）服务器在国内，低延迟；OpenAI/Claude 模型需代理（MVP 首选国内模型 Qwen/GLM/DeepSeek/Kimi 直连）。
> - 通知渠道：**邮件 SMTP**（587/465 端口，避开 25 封锁，配第三方 SMTP 服务）。企业微信/Server 酱保留可插拔接口。
> - 部署形态：**云端 VPS（单容器）**；反向代理 / HTTPS：**Nginx** + 证书（certbot 或同等方案）。
> - 磁盘加密：VPS 宿主机按云厂商能力评估。
