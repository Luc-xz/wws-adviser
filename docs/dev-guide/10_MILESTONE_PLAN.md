...
**退出条件**（验收窗口 2026-09-02 ~ 09-08 五个交易日，PRD §20 压缩决策；测法口径见 2026-09-01 补记）：

- [x] 凯利输入通过样本外校准；模型 Gateway 无权写 `p`（AC-07、技术架构 §25）。✅ `breakout-20 = calibrated_oos`（n_eff_oos=109/30，reliability 通过，有效期自动续至 2026-12-09）；结构性测试锁定 p 写路径仅在回测/校准服务
- [x] 硬风险上限始终截断理论值；移除校准标记后不再给凯利新增仓位（AC-07）。✅ 自动测试全覆盖（test_exit2/test_exit1）；窗口内 30 条真实建议全部 suspend 且零区间。凯利正向实例（突破触发）未出现，按 PRD §20 压缩决策为出窗项不阻塞
- [x] 行情过期/账本未对账/模型冲突时可靠进入降级，拒绝时不输出具体区间（PRD §18、技术架构 §25）。✅ 测试 + 真实实例：8/31 三重原因降级（ledger_unreconciled/data_stale/no_calibrated_signal，零区间）、窗口内 30 条全部不带数量
- [x] 盘中快照新鲜度达标率 ≥95%；用户快速建议阅读 ≤30 秒（PRD §4.2）。✅ 新鲜度 30/30=100%（≥25 样本、0 条 data_stale；9/2 诊断时段 38 条按验收规则剔除，时间戳/采样日志/部署时刻三方可查）；阅读时长 9/8 三卡抽测通过——000807 卡渲染至下一查询全程 28s（含输入），纯阅读远低于 30s
- [x] AC-03 全路径：过期行情 → 停止给具体交易数量。✅ 深市行情 market 硬编码 bug 修复（e8ba948）后沪深全标的验证，全部降级 suspend 零数量

> **Phase 2 完成声明（2026-09-08，VPS 114.132.239.95 实测收官）**：五条退出条件全绿。
> 运行镜像 `wws-adviser:8ef2a2a`（dev，含文符合规补齐三件：SSE /events、市场状态机、
> 建议有效期 600s）。验证窗口为 PRD §20 压缩决策的 5 个交易日（9/2~9/8，采样密度
> ≥5 次/日沪深混合）。**出窗持续项**：① 凯利正向实例——待持仓真实触发 20 日突破，
> 届时自然产出约束轨迹实证；② 建议评价回灌——首条可操作建议 +10 自然日进入评价
> 窗口，每日 16:30 review 任务持续积累；③ 被动运行形态数据。验证期发现并修复：
> advice TTL 缓存命中 AttributeError（合并期 mypy 捕获）、盘中行情 market 硬编码
> SSE（深市恒降级，全持仓实测发现）。20 交易日成功指标观察窗（PRD §4.2）继续至
> 完整窗口，非验收阻塞。
...
>   仅增量）；③ **文符合规补齐（8ef2a2a，2026-09-02）**：SSE `/api/v1/events`（工作项 2.2
>   首次交付）、市场状态机 `/market/state` 实接日历（工作项 2.1 占位移除）、建议有效期
>   10 分钟与缓存窗口解耦（PRD §20 决策对齐，原实现误为 5 分钟）。至此八个工作项全部
>   交付。剩余为观察窗积累——**验收窗口 2026-09-02 压缩为 5 个交易日**
> （2026-09-02 ~ 09-08，PRD §20 变更决策）：条件 4 采样密度要求为盘中问询 ≥5 次/日、
> 沪深混合、合计 ≥25 样本（≥95% 在该样本量下容错 1-2 条 data_stale），阅读 ≤30 秒于
> 9/8 验收日人工抽测（每形态 3 次）；条件 2 正向实例（突破触发）不因压缩免除。
> 9/8 收盘后按 Phase 1 口径核账验收。
...
仍留待后续：企微/Server酱真实联调（需凭据）。
（已清账：SSE `/events` 于 Phase 2 收口；离线报告私有缓存与报告导出于 Phase 3.4 交付；
交易录入/CSV 导入 UI 于视觉批次 V4 交付——2026-09-15 勘误，本行此前误标为仍留待。）
...
### 8.3 前端视觉对齐与补页批次（2026-09-09 起草，同日 V1–V5 交付 + 审计回填）

> Phase 3 验收期发现：前端语义层（ENFORCEMENT_CONTRACT 四层强制）正确，但视觉从未以设计稿为验收基准，且 UI §4.3 31 页仅实现 8 路由。本批次 = 视觉基准补齐（基准卡制度）+ 既有页对齐 + P0 缺页收官（含 §8.0 留白的 TX-02/TX-03 承接）。**详细计划：[VISUAL_ALIGNMENT_PLAN.md](../design-review/VISUAL_ALIGNMENT_PLAN.md)**。

| 波次 | 内容 | 估算 |
| --- | --- | --- |
| V1 基准固化 | 版式 token 化 + 基准卡模板 + 8 页基准卡 + 修 3 处演示稿硬伤 | 2 天 |
| V2 外壳与全局 | 桌面外壳（UI §5.2，现缺）+ 移动外壳对齐 + PageHeader + 全局版式 | 3 天 |
| V3 既有页对齐 | 8 页逐页对照基准卡（HOME/PORT/REP×3/DATA/CHAT/LIB/SET/AUTH） | 5 天 |
| V4 P0 缺页补齐 | TX-01/02/03、ACC-01、PORT-02、HOME-02、CHAT-02、SET-01~08、ONB-01（P2 可裁） | 8 天 |
| V5 走查收口 | 全页走查 + 深色核验 + **与上线门槛 PWA 真机项合流** | 2 天+真机 |

**排期落点**：主体（V1–V4）在 §7 上线门槛 PWA 真机验证之前；与观察窗/出窗项并行。总量 12–18 工作日（单人，含 P2 裁剪线余量）。

**执行结果（2026-09-09，分支 `visual-alignment`，V1–V5 + 批次审计回填）**：

- 路由 8→19（TX-01/02/03、ACC-01、PORT-02、SET-01/02/03/04/06/08 全交付）；桌面外壳（AppShellDesktop）+ 移动外壳按 UI §5 对齐；版式 token 化（fontSize 12 档 + radius 覆盖）；深色全页复扫清零；门禁全绿（eslint 仅 1 处论证豁免 `vue/no-v-html`——转义优先渲染器；vitest 36 例 0 跳过）。
- UI §18.4 移动 27 页达成 **24/27**（2026-09-10 更新）：原缺口 5 页中 HOME-02/CHAT-02 已随 `advice_records` 查询 API 交付收口（当日，dev）——后端 `GET /api/v1/advice`（列表，筛选+游标分页）+ `/{id}`（详情）+ `/{id}/evaluation`（评价回读）+ 前端两页（/advice 建议列表：今日分组+历史折叠+风险 Tab；/advice/:id 详情：区间条+凯利折叠+评价回读）+ 首页风险聚合条入口改指 /advice + 基准卡 2 张（19→21）；ONB-01/SET-05/07 维持 P2 裁剪线。
...
### 8.5 数据源条款复核（2026-09-15，PRD §18 / §7 上线门槛项）

| 数据面 | 供应商/接口 | 条款/性质 | 使用方式 | 结论 |
| --- | --- | --- | --- | --- |
| 日线/净值/日历 | AKShare（MIT，Copyright 2019-2026 Albert King） | 开源 MIT，接口聚合公开数据 | 端口+适配器；每日 15:20 持仓增量（30 天窗口）；单标的失败不重试轰炸 | ✅ 符合 |
| 盘中快照 | 腾讯行情 HTTP 接口（qt.gtimg.cn） | 公开未鉴权行情接口 | 盘中问询时按标的拉取（用户主动触发），无定时轮询 | ✅ 只读低频，符合业界惯例；不 redistribute 原始数据（仅派生建议） |
| 公告语料 | 东财公告中心接口（np-anotice/np-cnotice） | 公开未鉴权查询接口 | 每日 15:20 持仓+自选增量（14 天窗口）+ 用户一键采集；正文仅存本地供个人研究引用（含来源 URL+哈希可追溯） | ✅ 个人研究合理使用；引用链保留来源归属；不分发原文 |
| 模型 | 阶跃 step-3.7-flash（商业 API） | 商业付费 API | 网关统一调用，密钥仅 env | ✅ 符合 |
| 通知 | QQ 邮箱 SMTP | 个人账户 | 终态通知 | ✅ 符合 |

> 复核口径：MVP 为单用户个人工具，不对外提供服务、不转售/分发上游数据；
> 数据落盘均为本地（/data），引用输出附来源与哈希。若未来转对外服务，
> 需重评东财/腾讯接口的商业条款并升级 Tushare Pro（既定升级路径）。
