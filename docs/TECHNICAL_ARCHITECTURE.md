...
1. 验证概率来自允许的、样本外校准的信号版本。
2. 计算原始凯利 `f* = (b × p - (1 - p)) / b`。
3. 负值截断为 0，不生成做空建议。
4. 应用 0.10～0.25 的分数凯利折扣及置信/流动性折扣。
5. 依次应用现金、单标的、行业、组合波动和回撤约束。
6. 转换为目标仓位区间和可交易数量区间。
7. 按最小交易单位和费用校验；不能安全取整时只显示仓位区间。
8. 输出完整调整轨迹，供 UI 解释理论值为何变化。

任何模型文本或模型自报“置信度”都不能进入 `p`。
