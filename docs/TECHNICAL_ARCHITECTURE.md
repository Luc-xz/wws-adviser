...
- **初始渠道（MVP）**：邮件 SMTP（587 STARTTLS / 465 SSL，云端 VPS 须避开 25 端口封锁，配第三方 SMTP 服务）。
- **可插拔接口**：企业微信机器人（webhook）、Server 酱（token）保留为适配器，后续按需启用。
- 通知内容支持隐私模式（锁屏摘要不显示标的/金额），模板与报告 schema 版本绑定。
...
数据源未就绪或当日数据未发布时进入 `pending` 状态，不提前改变已确认持仓。
...
1. 08:30 创建业务日期任务并校验交易日。
2. 同步持仓相关公告、公司行动、日线和必要新闻。
3. 校验上一交易日数据完整性。
4. 冻结 08:30～任务截止时的数据快照。
5. 计算组合风险和每个标的候选动作。
6. 09:00 前发布；缺失源按降级规则发布数据异常摘要。
...
16:00 后先确认日线完整性，再计算收益归因和行为偏差。数据源缺失或日线未就绪时生成 `PARTIAL` 报告，数据补齐任务触发同一报告的新版本，保留旧版本而非覆盖。
...
```mermaid
flowchart LR
    Phone["手机 PWA"] -->|"HTTPS"| Nginx["Nginx 反向代理"]
    Nginx --> Container["WWS Adviser Container"]
    Container --> Volume["/data 持久化卷"]
    Container --> Internet["外部数据源与模型 HTTPS"]
```

- 仅开放 443，应用端口不直接公网暴露。
- Nginx 负责 HTTPS 终止（证书由 certbot 或同等方案管理）、登录限速、请求体大小限制和安全头。
- 应用自身仍启用登录，不把反代当作唯一认证。
- VPS 宿主机配置自动启动、磁盘监控和异地加密备份。
- `/data` 使用持久磁盘并执行异地备份。

### 17.3 可选：家庭设备/NAS + Tailscale
...
| 开市前报告 | 09:00 前完成 | 分阶段任务、来源超时、不无限等待 |
| 收市后报告 | 17:00 前完成 | 日线校验、异步模型、部分数据降级 |
...
7. 《部署与恢复手册》：云端 VPS（Nginx）部署、升级、备份及灾难恢复。
